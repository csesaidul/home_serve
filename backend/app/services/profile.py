...
def get_client_profile(db: Session, user_id: int) -> dict | None:
    row = db.execute(
        text("""SELECT u.id AS user_id, u.name, u.gender, u.phone, cp.profile_photo,
...
def update_client_profile(db: Session, user_id: int, name: str | None, gender: str | None, profile_photo: str | None, address: str | None) -> dict | None:
    if get_client_profile(db, user_id) is None:
        return None
    db.execute(
        text("UPDATE users SET name = COALESCE(:name, name), gender = COALESCE(:gender, gender) WHERE id = :user_id"),
